...
use super::{FilePath, checksum::Checksum, metadata_file::MetadataFile};
            ...
            match line {
                Line::FileEntry(v) => {
                    ...
                    if !files.contains_key(file_line.path) {
                        files.insert(
                            file_line.path.to_compact_string(),
                            FileEntry {
                                size: file_line.size,
                                md5: None,
                                sha1: None,
                    ...
                    match checksum_state {
                        ...
                        ChecksumState::No => {
                            return Err(MirsError::ParsingRelease {
                ...
                Line::Unknown => match checksum_state {
...
impl ReleaseFileFilter {
    ...
    pub fn accept(&self, path: &str) -> bool {
        ...
        let component = component.to_str().expect("path should be utf8");
        ...
        if parts.peek().is_none()
            && self
                .file_prefix_filter
                .iter()
                .any(|v| component.starts_with(v.as_str()))
        {
            return true;
        }
        ...
        while let Some(Component::Normal(part)) = parts.next() {
            let part_name = part.to_str().expect("path should be utf8");
            ...
            if parts.peek().is_none() {
                if self
                    .file_prefix_filter
                    .iter()
                    .any(|v| part_name.starts_with(v.as_str()))
                {
                    return true;
                }
...
impl Iterator for ReleaseFileIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.release
            .files
            .pop_first()
            .map(|(path, file_entry)| (path.into(), file_entry))
    }
}
...
impl<'a> FileLine<'a> {
    pub fn parse(value: &'a str) -> Result<Self> {
        ...
        let (Some(checksum), Some(size), Some(path)) = (parts.next(), parts.next(), parts.next())
        else {
            return Err(MirsError::ParsingRelease {
                line: value.to_compact_string(),
            });
        };
...
impl FileEntry {
    ...
    pub fn into_paths(
        ...
        file_path: &FilePath,
        by_hash: bool,
    ) -> Result<(Option<Checksum>, FilePath, Vec<FilePath>)> {
        ...
        let primary_target_path = if by_hash {
            let by_hash_base = FilePath(file_path.parent().unwrap_or("").to_compact_string());

            symlink_paths.push(file_path.clone());

            let strongest_checksum = checksum_iter
                .next()
