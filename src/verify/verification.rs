...
impl Step<VerifyState> for Verify {
    ...
    async fn execute(&self, ctx: Arc<Context<VerifyState>>) -> Result<StepResult<Self::Result>> {
        ...
        let progress_bar = progress.create_download_progress_bar().await;
        ...
        for (metadata_file, file_entry) in &mut metadata {
            ...
            ctx.state
                .verifier
                .queue(Arc::new(VerifyTask {
                    size: Some(size),
                    checksum: checksum.ok_or_else(|| MirsError::VerifyTask {
                        path: primary.clone(),
                    })?,
                    paths: vec![primary],
                }))
        ...
        let task_progress_bar = progress_bar.clone();
        ...
        spawn_blocking(move || {
            ...
            for meta_file in index_files {
                ...
                for entry in meta_file {
                    ...
                    async_handle.block_on(async { task_verifier.queue(verify_task).await })?;

                    task_progress.update_for_files(&task_progress_bar);
                }
            }
