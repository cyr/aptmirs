...
use pgp::composed::{
    CleartextSignedMessage, Deserializable, DetachedSignature, SignedPublicKey, SignedPublicSubKey,
};
...
pub fn read_public_key(path: &FilePath) -> Result<SignedPublicKey> {
    let key_file = std::fs::File::open(path).map_err(|e| MirsError::PgpPubKey {
        path: path.clone(),
        inner: Box::new(e.into()),
    })?;

    let (signed_public_key, _) =
        SignedPublicKey::from_reader_single(&key_file).map_err(|e| MirsError::PgpPubKey {
            path: path.clone(),
            inner: Box::new(e.into()),
        })?;
