...
use super::{MirrorState, verify_and_prune};
    ...
    async fn execute(&self, ctx: Arc<Context<MirrorState>>) -> Result<StepResult<Self::Result>> {
        ...
        for (mut file, file_entry) in release.into_iter() {
            ...
            if let Some(checksum) = file_entry.strongest_hash() {
                ...
                let checksum_path = FilePath(format_compact!(
                    "{by_hash_base}/{}",
                ...
                } else if (checksum_path.exists())
                    && file_path_in_root.exists()
                    && !ctx.cli_opts.force
                {
                    continue;
                }
        ...
        if ctx.progress.files.failed() > 0 {
            return Err(MirsError::InconsistentRepository {
                progress: ctx.progress.files.clone(),
            });
        }
